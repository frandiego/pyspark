...
os.umask(0o77)  # ensure that always chmod go-wrx
...
try:
    ...
    if "c.IPKernelApp.matplotlib = 'inline'" not in kernel_config:
        ...
        kernel_file.close()
        os.chmod(ipython_kernel_config_path, 0o600)
    ...
    if not os.path.exists(ipython_notebook_config_path) or 'open_browser = False' not in open(ipython_notebook_config_path).read():
        ...
        config_file.close()
        os.chmod(ipython_notebook_config_path, 0o600)
    ...
    if not os.path.exists(jupyter_kernel_config_path):
        ...
        config_file.close()
        os.chmod(jupyter_kernel_config_path, 0o600)
